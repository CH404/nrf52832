...
err_code = I2c_Tx_OneByte(MPU_ADDR,MPU_MOTION_DET_REG,0x01);
if(err_code != SUCCESS_I2C)
{
	while(1);
}

err_code = I2c_Tx_OneByte(MPU_ADDR,MPU_MDETECT_CTRL_REG,0x00);
err_code = I2c_Tx_OneByte(MPU_ADDR,0x20,0x01);
if(err_code != SUCCESS_I2C)
{
	while(1);
}
...
err_code = I2c_Tx_OneByte(MPU_ADDR,MPU_INTBP_CFG_REG,0x1C);
if(err_code != SUCCESS_I2C)
{
	while(1);
}
