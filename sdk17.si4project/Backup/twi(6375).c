...
G_CHECK_ERROR_CODE_INFO(err_code);
NRF_LOG_INFO("%d,%d,%d,%d",gesture_rxbuff[2],gesture_rxbuff[3],gesture_rxbuff[4],gesture_rxbuff[5]);
		}
...
#if 0
uint8_t tx1_buff[] = {0x20,0x02,0x04,0x01,0x11};
...
G_CHECK_ERROR_CODE_INFO(err_code);
if(rx1_buff[0] != 0x00 && rx1_buff[1] != 0x00)
{
    NRF_LOG_INFO("set interrupt error");
    while(1);
}
#endif 
