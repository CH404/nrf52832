...
//	BaseType_t pxHigherPriorityTaskWoken;
	...
	float yaw;
	uint8_t reg;
    reg = I2c_Rx_OneByte(MPU_ADDR,MPU_INT_STA_REG);
		NRF_LOG_INFO("MOT_Detect status = %d",reg);
                 reg = I2c_Rx_OneByte(MPU_ADDR,MPU_INTBP_CFG_REG);
                 NRF_LOG_INFO("MPU_INTBP_CFG_REG status = %d",reg);
	if(pin == MPU6050_INT_PIN)
		{
			if(action == NRF_GPIOTE_POLARITY_HITOLO)
				...
				{
				NRF_LOG_INFO("LOTOHI");
				//xSemaphoreGiveFromISR(MPURdyDate, &pxHigherPriorityTaskWoken);
					//	portYIELD_FROM_ISR(pxHigherPriorityTaskWoken);
				//	reg = I2c_Rx_OneByte(MPU_ADDR,0x3A);
			//	NRF_LOG_INFO("MOT_Detect status = %d",reg);
...
/*	float pitch;
 ...
 // reg = I2c_Rx_OneByte(MPU_ADDR,MPU_INT_EN_REG);
	//	NRF_LOG_INFO("MOT_Detect status = %d",reg);
              //     reg = I2c_Rx_OneByte(MPU_ADDR,MPU_INTBP_CFG_REG);
              //   NRF_LOG_INFO("MPU_Timer_CallBack status = %d",reg);
  reg= 	nrf_gpio_pin_read(MPU6050_INT_PIN);
NRF_LOG_INFO("%d",reg);
   reg = I2c_Rx_OneByte(MPU_ADDR,MPU_INT_STA_REG);
		NRF_LOG_INFO("MOT_Detect status = %d",reg);
                  reg= 	nrf_gpio_pin_read(MPU6050_INT_PIN);
NRF_LOG_INFO("%d",reg);
reg = mpu_get_dmp_state(&enable);
NRF_LOG_INFO("%d,%d",reg,enable);
}
...
void MPU_task(void *parameters)
{
	...
	while(1)
		{
			xSemaphoreTake(MPURdyDate,portMAX_DELAY);
			if(0 == mpu_dmp_get_data(&pitch,&roll,&yaw))
				{
					temp = MPU_Get_Temperature();
					MPU_Get_Accelerometer(&aacx,&aacy,&aacz);	//得到加速度传感器数据
