...
void ST7789_DrawPixel(uint16_t x, uint16_t y, uint16_t color)
{
	...
	uint8_t data[] = {color >> 8, color & 0xFF};
	ST7789_Select();
	SPI_WriteDataLength(data,sizeof(data));
	ST7789_UnSelect();
}
	...
	while(size)
		{
			if(size>65535)
				{
				  size -=65535;
				ST7789_WriteDataLength(&pic[8+remain],65535);
				remain += 65535;
				}
			else
				{
				ST7789_WriteDataLength(&pic[8+remain],size);
				size = 0;
				}
			
		}
//	ST7789_WriteDataLength(pic, sizeof(pic));
}
