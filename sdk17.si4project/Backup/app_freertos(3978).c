...
void LcdRefreshTaskHandler(void* paramenters)
{
	...
	while(1)
		...
		xTaskNotifyWait(0x00,ULONG_MAX,&notifyVakue,portMAX_DELAY);
              uint32_t a =   notifyVakue & (~MPU6050_TO_LCD_NOTIFY_BIT);
		//判断是那个任务发送通知
		if(notifyVakue & (~MPU6050_TO_LCD_NOTIFY_BIT)  == 0x00)
			{
		 ...
		 if(notifyVakue & (~IT725_TO_LCD_NOTIFY_BIT) ==0x00)
			{
				LCD_Refresh(datebuff,timebuff,&UI_Page,IT725_TO_LCD_NOTIFY_BIT,UI_Page);
                                return;
			}
		if(notifyVakue & (~RTC_TO_LCD_NOTIFY_BIT) ==0x00)
			{
...
void RTCUpdateTaskHandler(void *pvParamenters)
{
 ...
 //   if( xSemaphoreTake(RTCUpdateSem,portMAX_DELAY))
    	{
//			 NRF_LOG_INFO("NRFX_RTC");
