...
/*void peer_test(void)
{
	...
	ret_code_t err_code;
	pm_peer_id_t peerIdBuff;
//	pm_peer_id_list(pm_peer_id_t * p_peer_list, uint32_t * const p_list_size, pm_peer_id_t first_peer_id, pm_peer_id_list_skip_t skip_id);
err_code = pm_peer_id_get(m_conn_handle,&peerIdBuff);	
NRF_LOG_INFO("peer_test %d",err_code);
NRF_LOG_INFO("peer_test %d",peerIdBuff);
