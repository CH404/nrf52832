...
void MPU_INT_Init(void)
{
 ...
 I2c_Tx_OneByte(MPU_ADDR,MPU_MOTION_DET_REG,0x01);

I2c_Tx_OneByte(MPU_ADDR,MPU_MDETECT_CTRL_REG,0x00);
//I2c_Tx_OneByte(MPU_ADDR,MPU_INTBP_CFG_REG,0x0A);


//	I2c_Tx_OneByte(MPU_ADDR,MPU_INT_EN_REG,0x01);	//数据就绪中断
	I2c_Tx_OneByte(MPU_ADDR,MPU_INT_EN_REG,0x40);	//运动检测中断
}
...
MPU_Pin_int();
	...
	while(1)
		{
			a = nrf_gpio_pin_read(MPU6050_INT_PIN);
			NRF_LOG_INFO("a = %d",a);
