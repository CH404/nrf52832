...
void IT725X_handler(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
	  ret_code_t err_code;
...
IT725X_GetCommand(buffer_type,rx_buff,1);
	...
	void IT725X_InterruptInit(void)
{
	...
	nrf_drv_gpiote_in_init(IT725X_Int_Pin,&config,IT725X_handler);	
...
* 返回: it7251 错误码
****************************************/
uint16 IT725X_SetCommand(uint8_t *commandAndData,uint8_t length)
{
...
*返回值:
************************************************/
uint16_t IT725X_InterruptNotificationEnable(void)
{
	uint_t err_code;
	...
	if(err_code != IT725X_SUCCESS)
		return err_code;
	IT725X_GetCommand(&tx2_buff,sizeof(tx2_buff),&rx2_buff,sizeof(rx2_buff));
	if(rx2_buff[1] == tx1_buff[4] && rx2_buff[2] == tx1_buff[5])
		{
