...
#define BATTERY_SAADC_OFFSET 0.2
#if 0
...
void BatteryTimerBackCall(TimerHandle_t xTimer)
{
    ...
    battery_Value = battery *7.2/1024+BATTERY_SAADC_OFFSET;
   
#else
		 battery_Value = SAADC_Value*7.2/1024+0.2+BATTERY_SAADC_OFFSET;  
     SAADC_Start();	//启动电量检测，中断       
#endif

		
lcd_RefreshBattery(i++);
if(i>=4)
	i=0;
...
#if 0
		//计算
		battery_display = 420 - (uint16_t)(battery_Value *100);
		switch(battery_display/100)
			{
			case 1:	
#endif
}
...
//用于画 ... (hrs开启)
	...
	battery_timer = xTimerCreate("bty",1000,pdTRUE,(void*)2,BatteryTimerBackCall);
