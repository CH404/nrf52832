...
nrf_drv_gpiote_in_init(INT_PIN,&config,test_handler);
...
uint8_t tx1_buff[] = {0x20,0x02,0x04,0x01,0x10};
uint8_t rx1_buff[2] = {0xA0};
err_code = nrf_drv_twi_tx(&twi0,IT725X_ADDRESS,tx_buff,5,false);	//set interrupt
G_CHECK_ERROR_CODE_INFO(err_code);
tx_buff[0] = 0x80;
err_code = nrf_drv_twi_tx(&twi0,IT725X_ADDRESS,tx_buff,1,true);
G_CHECK_ERROR_CODE_INFO(err_code);
