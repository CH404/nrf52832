...
SPI_EXT ret_code_t SPI_WriteDataLength(const uint8_t *data,uint16_t length);
SPI_EXT uint8_t SPI_ReadByte(uint8_t command);
//void SPI_SoftWareInit(void);
//void SPI_WriteByte(uint8_t byte);
SPI_EXT void SPI_Rest(void);
SPI_EXT void SPI_Init(void);
SPI_EXT void SPI_Uninit(void);
#endif