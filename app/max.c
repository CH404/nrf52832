...
static const nrf_drv_twi_t max = NRF_DRV_TWI_INSTANCE(1);

static void twi1_handler(nrf_drv_twi_evt_t const *p_evt,void *p_context)
{
	switch(p_evt->type)
		{
...
static void TWI_Init(void)
{
 ...
 config.interrupt_priority = APP_IRQ_PRIORITY_HIGH;
  config.clear_bus_init     = false;
	err_code = nrf_drv_twi_init(&max,&config,twi1_handler,NULL);
...
{
	ret_code_t err;
	err = nrf_drv_twi_tx(&max,MAX_DRV_WADDRESS,&address,1,false);
		if(err != NRF_SUCCESS)
		return 1;
	else
		return 0;
